...
def go_to_each_course(dir_name, link):
    ...
    for each_video in list_course_videos:
        ...
        map_videos[file_name]=video_link
    print("loaded all videos links in hash map",map_videos)

    for name,v_link in map_videos.items():
        driver.get(v_link)
        ...
        print(f"done with file download:{name}")
    time.sleep(2)
